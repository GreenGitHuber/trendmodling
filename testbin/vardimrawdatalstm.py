...
from pca import PCA
from datahelp import read_same_day
import math
...
def split_dataset(dataset,time_step):
    ...
    for i in range(0,days-time_step):
        ...
        if d < 10:
            d_str = '0' + str(d)
        _,_,year_flow=read_same_day(3, 2017, '12', d_str,sensor_id)
        dataX.append(np.concatenate((dataset[i:i+time_step],year_flow),axis=0))
...
speed_data=r["flow"]
singel_sensor = speed_data[:,sensor_id]
m = singel_sensor.reshape(-1,288)  # 31*288
print(m.shape)
data = m
...
test_batch = math.ceil(batch*test_size)#向上取整,因为train_test_split函数就是向上取整的
train_batch=batch-test_batch#round四舍五入
...
#把数据划分为输入数据和输出数据
dataX,dataY = split_dataset(dataset_scaler,time_step=n_steps)  #dataX shape (28,6,288) ,dataY shape (28,1,288)
print(dataX.shape)
dataY=np.reshape(dataY,(batch,n_output))                       #dataY shape (28,1,288)
...
x_input = tf.placeholder(tf.float32, [None, n_steps+year_step, n_inputs])
