...
class PCA:
    def __init__(self, x, retain):
        ...
        self.u, self.s, _ = np.linalg.svd(sigma)  # u的每一行都是特征向量，s是特征值与u一一对应
        ...
        #         self.retain_num = i
        #         break
        self.main_vector = self.u[:, 0:self.retain_num + 1]  #取前k个特征向量
        self.rest_vector = self.u[:, self.retain_num + 1:]
        main_x_rot = np.matmul(self.x, self.main_vector)   #降维的数据
...
arr = [[-1,-1,0,2,0],[-2,0,0,1,1]]
arr = np.array(arr)

pca_obj = PCA(arr.T,0)

print (pca_obj.main_x)
print (pca_obj.rest_x)