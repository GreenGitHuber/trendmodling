...
flow_15 = np.zeros([h_15, v], dtype=np.float32)

for i in range(h_15):
    occupancy_15[i] = np.sum(occupancy_5[i * 3:i * 3 + 2], 0)
    speed_15[i] = np.sum(speed_5[i * 3:i * 3 + 2], 0)
    flow_15[i] = np.sum(flow_5[i * 3:i * 3 + 2], 0)


np.savez('../data/pems_speed_occupancy_15min.npz', speed=speed_15, occupancy=occupancy_15,flow=flow_15)
